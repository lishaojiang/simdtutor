...
#define MM_TRANSPOSE_3x4(__val0, __val1, __val2)                                                                       \
    do                                                                                                                 \
    {                                                                                                                  \
...
static void bench(benchmark::State& s)
{
    ...
    std::generate(
        img.begin(), img.end(),
        [uni = std::uniform_int_distribution<uint8_t>(), rng = std::mt19937()]() mutable { return uni(rng); });
#else
    std::generate(
        img.begin(), img.end(),
        [uni = std::uniform_int_distribution<uint32_t>(), rng = std::mt19937()]() mutable { return uni(rng); });
#endif
    for (auto _ : s)
    {
